...
esp_err_t i2s_parallel_setup(i2s_dev_t *dev, const i2s_parallel_config_t *cfg) {
    ...
    dev->timing.val=0;
    //When we use a bus transciever IC for gating CLK, OE and LAT, DATA maybe delayed.
    //Propagation time with a 74HCT24.. IC is about 50ns, which is the cycle time at 20MHz
    //So in this case, we may delay CLK with 1 cycle.
    //In this case, OE and LAT - will be delayed with CLK throught the IC, but this can be compensated with the driver.
    //dev->timing.tx_sd_out_delay = DATA_OUT_DELAY;
...
void i2s_parallel_resume(i2s_dev_t *dev)
{
    //dev->out_link.start = 1;
    dev->conf.tx_start  = 1;
}
void i2s_parallel_pause(i2s_dev_t *dev)
{
    //dev->out_link.start = 1;
    dev->conf.tx_start  = 0;
}
