...
int main(void)
{
	...
	InvnAlgoAGMConfig algo_parameters;
	memset(&algo_parameters,0,sizeof(algo_parameters));
	...
	/* Initialize algorithm */
	// algo_parameters.gyr_fsr = helper_gyro_fsr_bitfield_to_dps(ICM426XX_GYRO_CONFIG0_FS_SEL_2000dps);
	// algo_parameters.gyr_odr_us = ICM_GYRO_ODR_US;
	// algo_parameters.temp_sensitivity = 207;
	// algo_parameters.temp_offset = 25;
	// invn_algo_agm_init(&algo_parameters);
...
static int SetupMCUHardware(struct inv_icm426xx_serif * icm_serif)
{
	...
	*/
	 ...
	 */
	rc_cf_cb = inv_timer_configure_callback(FSYNC_EMULATOR_TIMER,
